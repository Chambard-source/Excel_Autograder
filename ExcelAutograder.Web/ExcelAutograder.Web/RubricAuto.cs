...
public static class RubricAuto
{
    ...
    private static (int col, int row) RulePos(Rule r)
    {
        // Prefer the explicit cell; else, use the first cell in the range
        string? a1 = r.Cell;
        if (string.IsNullOrWhiteSpace(a1) && !string.IsNullOrWhiteSpace(r.Range))
            a1 = r.Range.Split(':')[0];
        return A1ToKey(a1);
    }
        ...
        foreach (var (sheetName, sections) in sectionsPerSheet)
        {
            ...
            foreach (var (sectionName, ranges) in sections)
            {
                ...
                foreach (var ra in addrs)
                {
                    ...
                    {
                        var f = NormalizeFormula(cell.FormulaA1);
                        if (string.IsNullOrWhiteSpace(f)) continue;
    ...
    // Creates rules for cells/format/summaries inside the given ranges of one sheet.
